...
func main() {
	...
	for _, row := range rows {
		...
		ignore := false
		tls := false
		checkAbba := func() error {
			if l1 == l4 && l2 == l3 && l1 != l2 && l3 != l4 {
				if ignore {
					tls = false
					return errors.New("No ip7")
				}
				tls = true
				return nil
			}
		...
		for index := 4; index < len(row); index++ {
			err := checkAbba()
			if err != nil {
				break
			}

			current := string(row[index])
			if current == "[" {
				ignore = strings.LastIndex(row, "]") > index
			} else if current == "]" {
				ignore = false
			}
	...
	fmt.Printf("tls support: %d", counter)
}
