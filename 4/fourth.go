...
yhwooebeaz-nwilwcejc-ydkykhwpa-owhao-160[skuyi]`
	...
	for _, row := range rows {
		letterParse := &Letters{Counter: make(map[string]int)}
		sector := &Sector{}
		checksum := &CheckSum{}
		parsers := []Parser{
			sector,
		...
		previousLetter := ""
		for _, charByte := range row {
			char := string(charByte)
			for _, parser := range parsers {
				if parser.Parse(char, previousLetter) {
					break
				}
			}
			previousLetter = char
		...
		prevCounter := 0
		decoy := false
		min := letterParse.Counter[checksum.Letters[0]]
		for _, letter := range checksum.Letters {
			counter := letterParse.Counter[letter]
			if counter < min {
				min = counter
			}
		...
		if !decoy {
			...
			if !decoy {
				number, err := strconv.Atoi(sector.SectorId)
				if err == nil {
					result = result + number
				} else {
					log.Panicf("Failed to convert %s", sector.SectorId)
				}
			}
		}
	}
...
type Parser interface {
	Parse(letter string, previousLetter string) bool
}
...
func (letters *Letters) Parse(letter string, previousLetter string) bool {
	if letter == "-" || letter == "[" || letter == "]" {
		return false
	}
	count, ok := letters.Counter[letter]
	if ok {
		count = count + 1
		letters.Counter[letter] = count
	} else {
		letters.Counter[letter] = 1
	}
...
func (sector *Sector) Parse(letter string, previousLetter string) bool {
	if previousLetter == "-" {
		_, err := strconv.Atoi(letter)
		if err == nil {
			sector.IsNumber = true
		} else {
			return false
		}
	}
	if letter == "[" {
		sector.IsNumber = false
	}
	if sector.IsNumber {
		sector.SectorId = sector.SectorId + letter
		return true
	}
...
func (checkSum *CheckSum) Parse(letter string, previousLetter string) bool {
	if letter == "]" {
		checkSum.IsCheckSum = false
		return false
	}
	if previousLetter == "[" {
		checkSum.IsCheckSum = true
	}

	if checkSum.IsCheckSum {
		checkSum.Letters = append(checkSum.Letters, letter)
		return true
	}
	return false
}

func (checkSum *CheckSum) Exists(letter string) bool {
	for _, l := range checkSum.Letters {
		if l == letter {
			return true
		}
	}
